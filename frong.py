...
class Sourcecode(commands.Cog):
    ...
    async def frongonianunits(self, interaction: nextcord.Interaction):
      ...
      embed.add_field(name="**Definition:**", value="Think about it like this, you have 182 watermelons and each costs 6 quidd (Or 7 euros for my european folks. And 7 dollars for freedome lovers). in order to convert into frongonian units, you must take the squared derivative of each seed and calculate the megahertz it generates after a 15 ton, 31 year old mother, has sat on it. from there you can calculate the amount of politeness each person interacts with the mother. the more nice, the more canadian. convert it back to micro coulombs squared and you will get it in light years",inline=False)
      await interaction.response.send_message(embed=embed, ephemeral=False)
